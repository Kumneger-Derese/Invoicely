...
const getInvoice = asyncHandler(async (req, res, next) => {
  ...
  const { taxAmount, discountAmount } = await calculateInvoice(invoiceId);
  res.status(200).json({ ...invoice, discountAmount, taxAmount });
});
...
const createInvoice = asyncHandler(async (req, res, next) => {
  ...
  const { notes, currency, issueDate, dueDate, taxRate, discountRate, title } =
    req.body;
...
const updateInvoice = asyncHandler(async (req, res, next) => {
  ...
  if (discountRate) data.discountRate = parseInt(discountRate);
  if (taxRate) data.taxRate = parseInt(taxRate);
  ...
  await NotificationService.createNotification({
    ...
    message: `Invoice ${updatedInvoice.invoiceNumber} updated on 
        ${updatedInvoice.updatedAt.toLocaleDateString()}`,
    type: "warning",
    next,
  });
...
const deleteInvoice = asyncHandler(async (req, res, next) => {
  ...
  await NotificationService.createNotification({
    ...
    message: `Invoice ${deletedInvoice.invoiceNumber} deleted on 
        ${deletedInvoice.updatedAt.toLocaleDateString()}`,
    type: "warning",
    next,
  });
