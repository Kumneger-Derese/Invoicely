        ...
        <div className="p-8">
            <div className="flex justify-between px-4 mb-6 items-center">
                ...
                <div className="flex gap-x-4 items-center">
                    <Link to={'/notifications'} className="text-neutral-300 relative hover:text-lime-500"
                          title="Notification">
                        <HiOutlineBell size={28}/>
                        <span
                            className="absolute -top-2 -right-2 text-sm font-bold py-.5 px-2 rounded-full bg-white text-neutral-900">{notificationCount}</span>
                    </Link>
                        ...
                        <div key={invoice.id} className="p-4 bg-neutral-700 rounded-md flex flex-col gap-y-1">
                            ...
                            <div className="flex gap-x-4 items-center">
                                <Link to={`/edit-invoice/${invoice?.id}`}
                                      className="px-6 py-2 mt-4 rounded-md bg-lime-400 text-center font-semibold text-neutral-900">
                                    Edit
                                </Link>

                                <Link to={`/invoice-detail/${invoice?.id}`}
                                      className="px-6 py-2 mt-4 rounded-md bg-green-400 text-center font-semibold text-neutral-900">
                                    Detail
                                </Link>

                                <button
                                    onClick={() => handleInvoiceDelete(invoice.id)}
                                    className="px-6 py-2 mt-4 rounded-md bg-red-900 text-center font-semibold text-neutral-100">
                                    Delete
                                </button>
                            </div>
